...
def train(model, trainloader, optimizer, criterion_me, criterion_dr, criterion_multiclass, alpha=1, beta=1, gamma=1):
    # does one epoch of the training
    ...
    for i, data in tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        # loss from binary
        loss_me = criterion_me(outputs[:,0], label_me.float())
        loss_dr = criterion_dr(outputs[:,1], label_dr.float())
        ...
        # combine losses
        loss = alpha * loss_me + beta * loss_dr + gamma * loss_multiclass
        train_running_loss += loss.item()
        
        # change preds to be label-wise and combine binary + multiclass
        preds_binary = (torch.sigmoid(outputs[:,:2]) > 0.5).float()
...
def validate(model, validloader, criterion_me, criterion_dr, criterion_multiclass, alpha=1, beta=1, gamma=1):

    # does one validation epoch
...
if __name__ == '__main__':
    ...
    save_model_onnx(model, (3,IMAGE_SIZE,IMAGE_SIZE) ,args['pretrained'],name)
    save_feature_model_onnx(model, (3,IMAGE_SIZE,IMAGE_SIZE) ,args['pretrained'], name)
    save_plots(train_acc_me, valid_acc_me, train_acc_dr, valid_acc_dr, train_acc_glaucoma, valid_acc_glaucoma, train_loss, valid_loss, args['pretrained'], name)
